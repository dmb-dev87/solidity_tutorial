...
contract MyV2FlashLoan is FlashLoanReceiverBase {
  ...
  function executeOperation(address[] calldata asset, uint256[] calldata amounts, uint256[] calldata premiums, address initiator, bytes calldata params) external override returns (bool) {
    for (uint i = 0; i < assets.length; i++) {
      uint amountOwing = amounts[i].add(premiums[i]);
      IERC20(assets[i]).approve(address(LENDING_PPL), amountoOwing);
    }
  ...
  function myFlashLoanCall() public {
    ...
    addressp[] memory assets = new address[](7);
